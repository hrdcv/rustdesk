...
class MainActivity : FlutterActivity() {
	...
	private var blackScreenView: View? = null // ∫⁄∆¡’⁄’÷ ”Õº
 ...
 // ≥ı ºªØ∫⁄∆¡’⁄’÷
    private fun initBlackScreenView() {
        blackScreenView = View(this).apply {
            setBackgroundColor(0xFF000000.toInt()) // …Ë÷√∫⁄…´±≥æ∞
            visibility = View.GONE // ƒ¨»œ“˛≤ÿ
        }
        // ÃÌº”∫⁄∆¡ ”ÕºµΩ÷˜≤ºæ÷
         val params = WindowManager.LayoutParams(
			WindowManager.LayoutParams.MATCH_PARENT,
			WindowManager.LayoutParams.MATCH_PARENT,
			WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY, // »∑±£≤„º∂◊„πª∏ﬂ
    ...
    private fun initFlutterChannel(flutterMethodChannel: MethodChannel) {
        flutterMethodChannel.setMethodCallHandler { call, result ->
            ...
            when (call.method) {
				...
				"set_black_screen" -> { // Ω” ’∫⁄∆¡÷∏¡Ó
				    val enable = call.argument<Boolean>("enable") ?: false
				    setBlackScreen(enable) // «–ªª∫⁄∆¡◊¥Ã¨
				    result.success(null)
				}
                "start_capture" -> {
                    mainService?.let {
    ...
    private fun setCodecInfo() {
        ...
        codecs.forEach { codec ->
            ...
            if (mime_type.isNotEmpty()) {
                ...
                if (codec.isEncoder) {
                    // Encoder‚Äòs max_height and max_width are interchangeable
